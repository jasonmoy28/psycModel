...
{ 
  ...
  column_name = sapply(colnames(data_frame), function(x){text_convert(x,type = 'greek')}) %>% as.vector()
  colname_nchar = nchar(column_name)
  colname_nchar_max = nchar(column_name) %>% as.matrix() %>% t()
  data_frame_nchar_max = apply(data_frame, 2, function(x){max(nchar(x))}) %>% as.matrix() %>% t()
  combined_nchar_max = rbind(colname_nchar_max,data_frame_nchar_max)
  combined_nchar_max = apply(combined_nchar_max, 2, max) + 2
  data_frame_nchar = apply(data_frame, 2, function(x) {nchar(x)}) 
